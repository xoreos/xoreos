...
struct ShaderSampler {
	...
	ShaderSampler(GLuint id, uint32_t t) : glid(id), unit(t) {}
};
...
struct ShaderObject
{
	...
	uint32_t usageCount;
	uint32_t id;  // ID unique to a shader within its type (i.e unique across vertex shaders, or unique across fragment shaders). 0 is an invalid id.
...
struct ShaderProgram
{
	...
	uint64_t id;  // Set to (vertex.id << 32) | fragment.id
	GLuint glid;
	uint32_t usageCount;
...
public:
	...
	ShaderObject *getShaderObject(const Common::UString &filename, ShaderType type);
	...
	void bindShaderInstance(ShaderProgram *program, const void **v_vars, const void **f_vars);

	ShaderProgram *getShaderProgram(ShaderObject *v_obj, ShaderObject *f_obj);
	ShaderProgram *registerShaderProgram(ShaderObject *v_obj, ShaderObject *f_obj);
...
private:
	...
	void parseShaderVariables(const Common::UString &shaderString, std::vector<ShaderObject::ShaderObjectVariable> &var_list);
