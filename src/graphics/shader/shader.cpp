...
ShaderObject *ShaderManager::getShaderObject(const Common::UString &filename, ShaderType type) {
	...
	std::map<Common::UString, Shader::ShaderObject *>::iterator it = _shaderObjectMap.find(filename);
	if (it != _shaderObjectMap.end()) {
		...
		// The file could be loaded. Parse the file, and add it to the hash and list if the parse was valid.
		...
		glShaderSource(shaderObject->glid, 1, (const GLchar **)(&text), NULL);
		glCompileShader(shaderObject->glid);


		status("shader %s loaded", filename.c_str());
		...
		if (gl_status == GL_FALSE) {
			...
			_shaderObjectMap.insert(std::pair<Common::UString, ShaderObject *>(filename, shaderObject));
			parseShaderVariables(shaderString, shaderObject->variablesSelf);
			genShaderVariableList(shaderObject, shaderObject->variablesCombined);
			if(shaderObject->type == SHADER_VERTEX) {
				shaderObject->id = _counterVID++;
			}
			else {
				shaderObject->id = _counterFID++;
			}
		}
	}
...
void ShaderManager::bindShaderInstance(ShaderProgram *prog, const void **v_vars, const void **f_vars) {
	glUseProgram(prog->glid);
	for (uint32_t i = 0; i < prog->vertexObject->variablesCombined.size(); i++) {
		bindShaderVariable(prog->vertexObject->variablesCombined[i], prog->vertexVariableLocations[i], v_vars[i]);
	}

	for (uint32_t i = 0; i < prog->fragmentObject->variablesCombined.size(); i++) {
		bindShaderVariable(prog->fragmentObject->variablesCombined[i], prog->fragmentVariableLocations[i], f_vars[i]);
	}
}
...
ShaderProgram *ShaderManager::getShaderProgram(ShaderObject *vertexObject, ShaderObject *fragmentObject) {
	...
	ShaderProgram *rvalue = NULL;  // Default to nothing found.
...
ShaderProgram *ShaderManager::registerShaderProgram(ShaderObject *vertexObject, ShaderObject *fragmentObject) {
	...
	if (program != NULL)
		...
		return NULL;  // One id or the other is invalid - shader program can not be created.
	...
	uint64_t key = (((uint64_t)(vertexObject->id)) << 32) | (uint64_t)(fragmentObject->id);
	uint32_t shinyhappypeople = 0;
	while ((shinyhappypeople < _shaderProgramArray.size()) && (_shaderProgramArray[shinyhappypeople]->id < key)) {
		shinyhappypeople++;
	}
	...
	for (unsigned int i = (_shaderProgramArray.size() - 1); i > shinyhappypeople; i--) {
		_shaderProgramArray[i] = _shaderProgramArray[i - 1];
	}
	_shaderProgramArray[shinyhappypeople] = program;
	...
	status("processing vertex variables, count: %u", vertexObject->variablesCombined.size());
