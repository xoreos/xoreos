...
ERFWriter::ERFWriter(uint32 id, uint32 fileCount, Common::SeekableWriteStream &stream, ERFVersion version,
	...
	static const uint32 kLocStringTableOffset = 160; // 160 is the size of the erf header.
...
void ERFWriter::add(const Common::UString &resRef, FileType resType, Common::ReadStream &stream) {
	...
	// Write the actual resource data and add the size to the offset
	_stream.seek(_offsetToResourceData);
	size_t size = _stream.writeStream(stream);
