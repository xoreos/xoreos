...
/** @file
 *  Neverwinter Nights 2 engine functions operating on strings.
 */
...
void Functions::printObject(Aurora::NWScript::FunctionContext &ctx) {
	...
	status("NWN2: object<%s,%p)", Aurora::NWScript::formatTag(object).c_str(), static_cast<void *>(object));
}
...
void Functions::printVector(Aurora::NWScript::FunctionContext &ctx) {
	...
	status("NWN2: %s%f, %f, %f", prepend ? "PRINTVECTOR:" : "", x, y, z);
}
