...
void GUI::load(const Common::UString &resref) {
	if (!Common::isMainThread()) {
		Events::MainThreadFunctor<void> functor(boost::bind(&GUI::load, this, resref));
...
void GUI::loadWidget(const Aurora::GFFStruct &strct, Widget *parent) {
	...
	if (ctx.parent) {
		...
		ctx.widget->setPosition(floor(x), floor(y), z);
	} else {
		float x, y, z;
		getPosition(x, y, z);
	...
	if (label && ctx.strct->hasField("Obj_Caption")) {
		...
		float labelZ = ctx.strct->getDouble("Obj_Label_Z") - 5.0;
		...
		label->movePosition(floor(labelX), floor(labelY), -labelZ);
	}
...
bool GUI::createWidget(WidgetContext &ctx) {
	// ...BioWare...
...
WidgetLabel *GUI::createCaption(const Aurora::GFFStruct &strct, Widget *parent) {
	...
	float pX, pY, pZ;
	parent->getPosition(pX, pY, pZ);
	label->setPosition(floor(pX), floor(pY), pZ);
