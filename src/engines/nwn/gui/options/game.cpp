...
OptionsGameMenu::OptionsGameMenu(bool isMain) : _isMain(isMain), _gorepass(0), _feedback(0) {
...
void OptionsGameMenu::setVisible(bool visible) {
	if (visible) {
...
void OptionsGameMenu::callbackActive(Widget &widget) {
	...
	if (widget.getTag() == "PasswordButton") {
		if (!_gorepass)
			_gorepass = new OptionsGorePasswordMenu(_isMain);
	...
	if (widget.getTag() == "FeedbackButton") {
		if (!_feedback)
			_feedback = new OptionsFeedbackMenu(_isMain);
