...
OptionsSoundMenu::OptionsSoundMenu(bool isMain) : _isMain(isMain), _advanced(0) {
...
void OptionsSoundMenu::setVisible(bool visible) {
	if (visible) {
...
void OptionsSoundMenu::callbackActive(Widget &widget) {
	...
	if (widget.getTag() == "AdvSoundBtn") {
		if (!_advanced)
			_advanced = new OptionsSoundAdvancedMenu(_isMain);
