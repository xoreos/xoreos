...
/** @file
 *  Dragon Age: Origins engine functions operating on strings.
 */
...
void Functions::printToLog(Aurora::NWScript::FunctionContext &ctx) {
	...
	status("DragonAge: %s: %s", tstamp.c_str(), ctx.getParams()[0].getString().c_str());
}
...
void Functions::logTrace(Aurora::NWScript::FunctionContext &ctx) {
	...
	status("DragonAge: %s: [%d]<%s>: %s", tstamp.c_str(), ctx.getParams()[0].getInt(),
...
void Functions::printInteger(Aurora::NWScript::FunctionContext &ctx) {
	status("DragonAge: %d", ctx.getParams()[0].getInt());
}
...
void Functions::printVector(Aurora::NWScript::FunctionContext &ctx) {
	...
	status("DragonAge: %s%f, %f, %f", prepend ? "PRINTVECTOR:" : "", x, y, z);
}
...
void Functions::intToChar(Aurora::NWScript::FunctionContext &ctx) {
	ctx.getReturn() = Common::UString((uint32_t) (ctx.getParams()[0].getInt() & 0x7F), 1);
}
