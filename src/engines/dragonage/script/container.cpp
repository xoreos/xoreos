...
void ScriptContainer::enableEvents(bool enabled) {
	...
	for (size_t i = (int) kEventTypeSpellCastAt; i < kEventTypePartyPickerInit; i++)
		_eventEnabled[(EventType) i] = enabled;

	for (size_t i = (int) kEventTypeTrainingBegin; i < kEventTypeTrainingMovementCommandIssued; i++)
		_eventEnabled[(EventType) i] = enabled;
}
