...
#include "src/engines/jade/script/function_tables.h"
...
Aurora::NWScript::Object *Functions::getParamObject(const Aurora::NWScript::FunctionContext &ctx, size_t n) {
	Jade::Object *object = Jade::ObjectContainer::toObject(ctx.getParams()[n].getObject());
...
void Functions::jumpTo(Jade::Object *object, Area *area, float x, float y, float z) {
	// Sanity check
	...
	if (object->getArea() != area) {
		...
		Jade::Object *pc = Jade::ObjectContainer::toObject(_game->getModule().getPC());
		if (pc) {
