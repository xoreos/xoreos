...
/** @file
 *  The Witcher engine functions operating on strings.
 */
...
void Functions::sendMessageToPC(Aurora::NWScript::FunctionContext &ctx) {
	...
	warning("Send message to PC \"%s\": \"%s\"", pc->getName().getString().c_str(), msg.c_str());
}
...
void Functions::printFloat(Aurora::NWScript::FunctionContext &ctx) {
	...
	status("Witcher: %s", formatFloat(value, width, decimals).c_str());
}
