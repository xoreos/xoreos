...
GTEST_TEST(MemoryWriteStream, write) {
	...
	byte data[std::size(writeData) - 1] = { 0 };
	Common::MemoryWriteStream stream(data);

	EXPECT_EQ(stream.size(), std::size(writeData) - 1);
	EXPECT_EQ(stream.pos(), 0);

	const size_t writeCount = stream.write(writeData, std::size(writeData));
	EXPECT_EQ(writeCount, std::size(data));

	EXPECT_EQ(stream.size(), std::size(writeData) - 1);
	EXPECT_EQ(stream.pos(), std::size(writeData) - 1);
...
GTEST_TEST(MemoryWriteStreamDynamic, write) {
	...
	ASSERT_EQ(stream.size(), std::size(data));
